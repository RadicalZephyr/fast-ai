...
ProbeControl::ProbeControl(BWAPI::Unit *newProbe):myProbe(newProbe),
	myTiles(Broodwar->getStartLocations()),
	it(myTiles.begin())
{

}

bool ProbeControl::addProbe() {
	if (myProbe == 0) {
		//Broodwar->printf("ERROR: addMiner: null pointer");
		return false;
	}
	
	Broodwar->printf("%i", myTiles.size());
	TilePosition myPlace = Broodwar->self()->getStartLocation();
	myPlace.makeValid();
	
	//Position newPlace(*myTiles.begin());
	for(it = myTiles.begin(); it != myTiles.end(); it++){
		//Broodwar->printf("%i", (int)myTiles.size());
		if((*it) == myPlace)
			myTiles.erase(it);
	}
	it = myTiles.begin();
	Position nextPlace(*it);
	nextPlace.makeValid();
	myProbe->move(nextPlace);
	return true;
}

bool ProbeControl::inLocation(){
	if(myProbe->getPosition() == myProbe->getTargetPosition())
		return true;
	return false;
}

void ProbeControl::nextLocation(){
	it++;
	Position nextPlace(*it);
	nextPlace.makeValid();
	myProbe->move(nextPlace);
