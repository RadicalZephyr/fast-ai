...
public:
    ...
    void setShouldBuild(bool newFlag) {
        m_shouldBuild = newFlag;
    }

    void onFrame(void);
    void onFriendlyUnitDestroy(BWAPI::Unit *unit) {
        removeProbeFromGatherers(unit);
    }
    void onNeutralUnitDestroy(BWAPI::Unit *unit) {
   m_gas.remove(unit);
